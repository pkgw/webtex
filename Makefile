...
# The frontend, which drives the Web Worker engine and renders the output into
# the DOM. This can only be built after the bundle, because the frontend code
# embeds the list of glyph identifiers, which is generated as font files in
# the bundle are processed. The frontend does *not* include all of the code
# for the TeX engine; it only has code to display the intermediate ("chrome")
# data format generated by the worker.
...
| $(builddir)
	...
	rm -f $(builddir)/$$dstem $@ ; \
	cp $(builddir)/worker-webtex.js $(builddir)/webtex-frontend.js \
